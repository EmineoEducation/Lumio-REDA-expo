...
function BrowserApp({openTab,openPortfolio}){
  ...
  var [activeTab,setActiveTab]=React.useState(openPortfolio?'portfolio':(openTab||'article-moodwork'));

  React.useEffect(function(){
    var id=setInterval(function(){if(window.LUMIO_PORTFOLIO_DATA){setPR(true);clearInterval(id);}},1000);
    return function(){clearInterval(id);};
  },[]);

  React.useEffect(function(){
    window.__openAppInBrowser=function(type){
      if(type==='portfolio') setActiveTab('portfolio');
    };
    return function(){window.__openAppInBrowser=null;};
  },[]);

  if(openPortfolio&&activeTab!=='portfolio') setActiveTab('portfolio');

  var article=D.pressArticles?.[0]||{};
  var fausse=D.fausseUne||{};

  var TABS=[
    {id:'article-moodwork',favicon:'LE',fc:'#0a3d62',host:'lesechos.fr',title:'Moodwork signe Generali IDF · Les Échos',url:'https://lesechos.fr/tech-medias/wearables-sante-moodwork-generali-idf',type:'article'},
    {id:'fausse-une',      favicon:'LE',fc:'#0a3d62',host:'lesechos.fr',title:"Wearables santé : Moodwork s'impose en IDF",url:'https://lesechos.fr/sante/wearables-moodwork-idf',type:'fausse-une'},
    {id:'lumio-site',      favicon:'L', fc:'#0a7a6e',host:'lumio-health.com',title:'Lumio Health · Mesurez le stress invisible',url:'https://lumio-health.com',type:'corporate'},
    ...(portfolioReady?[{id:'portfolio',favicon:'📋',fc:'#1C2B3A',host:'portfolio.lumio-interne.fr',title:'Portfolio de compétences · BC01 REDA',url:'https://portfolio.lumio-interne.fr/bc01-reda',type:'portfolio'}]:[]),
  ];
    ...
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'#f0ede6'}}>
      ...
      <div style={{background:'#e8e4dc',borderBottom:'1px solid rgba(20,24,36,.1)',display:'flex',overflowX:'auto',padding:'4px 6px 0',gap:2,flexShrink:0}}>
        {TABS.map(function(t){return (
          <div key={t.id} onClick={function(){setActiveTab(t.id);}} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:'6px 6px 0 0',background:activeTab===t.id?'white':'transparent',cursor:'pointer',maxWidth:200,flexShrink:0,borderBottom:activeTab===t.id?'1px solid white':'none',marginBottom:activeTab===t.id?-1:0}}>
            ...
            <span style={{fontSize:11,color:activeTab===t.id?'var(--ink)':'var(--ink-mute)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.title}</span>
          </div>
        );})}
      </div>
      {/* URL bar */}
      ...
      <div style={{flex:1,overflowY:'auto'}}>
        {tab.type==='article'&&<div style={{maxWidth:720,margin:'0 auto',padding:'32px 24px'}}>
        ...
        {tab.type==='fausse-une'&&<div style={{background:'#0a1628',minHeight:'100%'}}>
          <div style={{background:'#0a1628',padding:'12px 24px',display:'flex',alignItems:'center',gap:12,borderBottom:'1px solid rgba(255,255,255,.1)'}}>
            <div style={{fontSize:18,fontWeight:800,color:'white',letterSpacing:'-.02em',fontFamily:'Georgia,serif'}}>Les Échos</div>
        ...
        {tab.type==='corporate'&&<div style={{background:'#0B2B2D',minHeight:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>
        ...
        {tab.type==='portfolio'&&(function(){
          ...
          return <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',color:'var(--ink-faint)',fontStyle:'italic',padding:40,textAlign:'center'}}>Portfolio disponible après soumission du livrable.</div>;
        })()}
      </div>
    </div>
  );
